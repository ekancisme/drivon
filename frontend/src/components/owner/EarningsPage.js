...
const EarningsPage = () => {
  ...
  const filteredTransactions = transactions.filter((tx) => {
    ...
    const matchesStatus =
      !statusFilter ||
      (tx.status && tx.status.toLowerCase() === statusFilter.toLowerCase());
    const matchesSearch =
      !search ||
      (tx.carName && tx.carName.toLowerCase().includes(search.toLowerCase())) ||
      (tx.renterName &&
        tx.renterName.toLowerCase().includes(search.toLowerCase()));
      ...
      >
        ...
        >
          {filteredTransactions.length > 0
            ? `Hiển thị ${indexOfFirstTransaction + 1}-${Math.min(
                indexOfLastTransaction,
    ...
    try {
      ...
      const percentageChange =
        earnings.lastMonthEarnings > 0
          ? (
              ((earnings.monthlyEarnings - earnings.lastMonthEarnings) /
      ...
      const fileName = `earnings_report_${selectedMonth.replace(
        "-",
        "_"
  ...
  const handleWithdraw = async () => {
    ...
    if (earnings.balance <= 0) {
      showErrorToast(
        "Không có tiền để rút! Tổng tiền thu qua hệ thống: " +
    ...
    if (withdrawAmountFloat > availableAmount) {
      showErrorToast(
        `Số tiền rút vượt quá tổng tiền thu qua hệ thống! Tối đa: ${availableAmount} ₫`
      );
      return;
    }
  ...
  const processDebtPayment = async () => {
    try {
      ...
      if (paymentResponse.data.data && paymentResponse.data.data.checkoutUrl) {
        ...
        showErrorToast(
          "Không thể chuyển hướng đến trang thanh toán. Vui lòng thử lại."
        );
      }
    } catch (error) {
  ...
  const handleConfirmDebt = async () => {
    ...
    try {
      await axios.post(`${API_URL}/debt-payment/confirm`, {
  ...
  return (
    <div className="earnings-page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Track your rental income and transactions
          </p>
        </div>
      ...
      <div className="earnings-summary"></div>
      ...
      <div className="earnings-overview">
        <div className="earnings-card total-earnings">
          <div className="card-content">
            ...
            <p
              ...
              style={{
                color: percentageChange >= 0 ? "#fff" : "#e74c3c",
                fontWeight: "bold",
              }}
            >
              {percentageChange > 0 ? "+" : ""}
        ...
        <div className="earnings-card total-profit">
          <div className="card-content">
            ...
            >
              {formatCurrency(earnings.totalProfit)}
            </p>
            <p style={{ color: "#000" }} className="comparison">
              Lợi nhuận banking sau khi trừ phí 2% mỗi giao dịch
            </p>
          </div>
          <i
            style={{ color: "#000" }}
            className="fas fa-piggy-bank card-icon"
          ></i>
        </div>
        <div className="earnings-card total-debt">
          <div className="card-content">
            ...
            >
              {formatCurrency(earnings.totalDebt)}
            </p>
            <p style={{ color: "#000" }} className="comparison">
              Nợ hệ thống sau 2% mỗi giao dịch cash
            </p>
          </div>
          <i
            style={{ color: "#000" }}
            className="fas fa-exclamation-triangle card-icon"
          ></i>
        </div>
        <div className="earnings-card total-profit">
          <div className="card-content">
            ...
            <p style={{ color: "#000" }} className="comparison">
              Tổng tiền thu qua hệ thống sau khi trừ nợ
            </p>
          </div>
          <i
            style={{ color: "#000" }}
      ...
      <div className="transactions-section">
        <div className="transactions-header">
          <h3>
            <i className="fas fa-credit-card"></i> Recent Transactions
          </h3>
            ...
            {currentTransactions && currentTransactions.length > 0 ? (
              currentTransactions.map((transaction) => (
                <tr key={transaction.id}>
                  ...
                  <td>
                    <i className="fas fa-car icon-prefix"></i>{" "}
                    {transaction.carName}
                  </td>
                  <td>
                    <i className="fas fa-user icon-prefix"></i>{" "}
                  ...
                  <td>
                    <span
                      className={`payment-method-badge payment-${transaction.paymentMethod?.toLowerCase()}`}
                    >
                      <i
        ...
        >
          ...
          >
            ...
            <input
              ...
              placeholder={`Amount to withdraw (Max: ${formatCurrencyExact(
                earnings.balance
              )})`}
            ...
            <div style={{ fontSize: "12px", color: "#666", marginBottom: 12 }}>
              Total earnings through system:{" "}
              {formatCurrencyExact(earnings.balance)} (exact: {earnings.balance}{" "}
              ₫)
            </div>
            <textarea
        ...
        >
          ...
          >
            ...
            <p style={{ color: "#666", marginBottom: 20 }}>
              Bạn sẽ được chuyển hướng đến trang thanh toán ngân hàng để thanh
              toán số nợ này. Sau khi thanh toán thành công, số nợ sẽ được xóa
      ...
      {showConfirmDebtModal && (
        ...
        >
          ...
          >
            ...
            >
              <b>Thanh toán nợ thành công qua PayOS!</b>
              <br />
              Số tiền nợ sẽ chỉ được trừ khỏi tài khoản khi bạn bấm{" "}
              <b>Xác nhận</b>.<br />
              <span style={{ color: "#e74c3c", fontWeight: 600 }}>
                Số tiền nợ: {formatCurrency(earnings.totalDebt)}
              </span>
            </div>
            <p style={{ marginBottom: 20, color: "#333", textAlign: "center" }}>
              Vui lòng kiểm tra lại số tiền và bấm <b>Xác nhận thanh toán nợ</b>{" "}
              để hoàn tất trừ nợ trên hệ thống.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: 12 }}>
              ...
              <button
                onClick={() => setShowConfirmDebtModal(false)}
                style={{
                  background: "#eee",
                  color: "#333",
                  border: "none",
