...
public class EarningsService {
    ...
    public EarningsReportDto getEarningsReport(Long ownerId, YearMonth yearMonth) {
        ...
        double totalBanking = payments.stream()
                .filter(p -> "PAID".equalsIgnoreCase(p.getStatus()))
                .filter(p -> "BANK".equalsIgnoreCase(p.getPaymentMethod())
                        || "BANK_TRANSFER".equalsIgnoreCase(p.getPaymentMethod()))
                .filter(p -> p.getPaymentDate() != null && YearMonth.from(p.getPaymentDate()).equals(yearMonth))
                .mapToDouble(Payment::getAmount)
                .sum();
        ...
        return new EarningsReportDto(totalEarnings, monthlyEarnings, pendingPayouts, lastMonthEarnings, totalCash,
                totalBanking, totalProfit, totalDebt, balance, transactions);
    }
    ...
    public Map<String, Object> getSystemStatistics() {
        ...
        double lastMonthSystemRevenue = systemRevenueRepository.getMonthlyRevenue(lastMonthStart,
                currentMonthStartForLast);
    ...
    public void recordSystemRevenue(Payment payment, Long ownerId) {
        ...
        if (!systemRevenueRepository.existsByBookingIdAndTransactionType(payment.getBookingId(), transactionType)) {
            ...
            systemRevenue.setTransactionDate(
    ...
    /**
     * Trả về danh sách doanh thu từng tháng trong năm hiện tại (hoặc 12 tháng gần
     * nhất)
     */
